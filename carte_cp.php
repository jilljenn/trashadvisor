...
while ($row = $res->fetch_assoc()) { 		
	...
	$sql1 = 'SELECT DISTINCT b.`code_INSEE` as code_INSEE, a.`code_postal`, b.`code_postal` as `ID,C,5`, c.`NOM_COM,C,50`, c.`X_CENTROID,N,7,0`, c.`Y_CENTROID,N,7,0` 
			FROM `couleur_dechet` a RIGHT JOIN `code_insee_postal` b ON a.`code_postal` = b.`code_postal` LEFT JOIN `commune_geofla` c ON  b.`code_INSEE` = c.`INSEE_COM,C,5`
	...
	while ($row1 = $res1->fetch_assoc()) { 	
		...
		$sql2 = 'SELECT couleur_dechet.dechet_type, couleur_dechet.conteneur_couleur, count(*) as nb 
					FROM couleur_dechet
					WHERE couleur_dechet.code_postal = "'.$row1['code_postal'].'" AND couleur_dechet.dechet_type = "'.$row['dechet_type'].'" 
