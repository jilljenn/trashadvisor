...
L.Control.Search = L.Control.extend({
	...
	options: {
		...
		text: 'Entrer le nom de la commune...',			//placeholder value	
...
//////end DOM creations
	...
	_defaultFilterJSON: function(json) {	//default callback for filter data
		...
		for(var i in json) {
         jsonret[ json[i][propName] ]= {};
		   if( L.Util.isArray(propLoc) )
			   jsonret[ json[i][propName] ].loc= L.latLng( json[i][ propLoc[0] ], json[i][ propLoc[1] ] );
         else
			   jsonret[ json[i][propName] ].loc= L.latLng( json[i][ propLoc ] );
			jsonret[ json[i][propName] ].id= json[i][ "id" ] ;
			jsonret[ json[i][propName] ].classr= json[i][ "classr" ] ;
			jsonret[ json[i][propName] ].name= json[i][ "name" ] ;
      }
...
//	in this mode every record can have a free structure of attributes, only 'loc' is required
	...
	_handleSubmit: function() {	//button and tooltip click and enter submit
      ...
      if (this._tooltip.hasChildNodes())
         firstTip = this._tooltip.childNodes[0]._text;
		...
		else
		{
            ...
            else if (firstTip && (loc=this._getLocation(firstTip))) {
               this._input.value = firstTip;
					this.showLocation(loc, this._input.value);
            }
				else {
               this.showAlert();
	...
	showLocation: function(loc, title) {	//set location on map from _recordsCache
			
		if(this.options.zoom)
			this._map.setView(loc.loc, this.options.zoom);
		else
			this._map.panTo(loc.loc);
      if(loc.geojson === undefined) {
         if (window.XMLHttpRequest === undefined) {
            window.XMLHttpRequest = function() {
               try { return new ActiveXObject("Microsoft.XMLHTTP.6.0"); }
               catch  (e1) {
         ...
         var request = new XMLHttpRequest();
         var url =  "https://cartoradon.irsn.fr/commune.py/communes/" +loc.id;
         var response = {};
         ...
         request.onreadystatechange = function() {
             if(request.readyState === 4 && request.status === 200) {
               response = window.JSON ? JSON.parse(request.responseText) : eval("("+ request.responseText + ")");
               loc.geojson = response;
               geojsonLayer.clearLayers();
               geojsonLayer.addData(loc.geojson);
            }
            var popup = L.popup().setLatLng(loc.loc).setContent('<b>'+loc.name+':</b><br/>'+texts[loc.classr]).openOn(map);
		...
		this.fire("search_locationfound", {latlng: loc.loc, text: title});
...
function search_by_click(ll) {
   ...
   while(lng<-180) lng += 360;
   var url =  "https://cartoradon.irsn.fr/commune.py/communes/lonlat/" + lng + ";" + ll.lat;
   var response = {};
   ...
   request.onreadystatechange = function() {
      if(request.readyState === 4 && request.status === 200) {
         ...
         var popup = L.popup().setLatLng(ll).setContent('<b>'+response.properties.name+':</b><br/>'+texts[response.properties.classr]).openOn(map);
      }
